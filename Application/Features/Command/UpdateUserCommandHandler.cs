...
namespace ClientManagement.Application.Features.Command
{
    public class UpdateUserCommandHandler(ClientManagementDbContext dbContext, ICurrentUserService currentUserService) : IRequestHandler<UpdateUserCommand, string>
    {
        ...
        {
            if (currentUserService.clientId < 1) return "unauthorized";

            var status = await dbContext.Works.Where(id => id.ClientId == currentUserService.clientId).AnyAsync(paid => paid.IsPaid == false, cancellationToken);
            if (status == true) return "Please complete all the payments before proceeding.";

            var userDetails = await dbContext.UserDetails.Where(x => x.Id == currentUserService.clientId).FirstOrDefaultAsync(cancellationToken);
            ...
            int linesChanged = await dbContext.SaveChangesAsync();
